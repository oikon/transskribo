...
**Done when:** `transskribo export --all` exports in all available formats. `transskribo pipeline --config config.toml` runs the full end-to-end workflow (transcribe → enrich → export all) in a single command. Partial failures in any stage do not prevent subsequent stages from running. Tests pass, lint clean, types clean.
