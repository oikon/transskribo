...
- [x] 14.01 — Split config: Remove `template_path` and `transcritor` from `EnrichConfig`. Define `ExportConfig` frozen dataclass in `config.py` with fields: template_path (default: Path("templates/basic.docx")), transcritor (default: "Transcrição automática (IA)"). Implement `load_export_config(file_config: dict, cli_overrides: dict) -> ExportConfig` that reads the `[export]` section from the TOML dict and merges CLI overrides. Add `_EXPORT_DEFAULTS` dict. Update `config.example.toml`: move `template_path` and `transcritor` from `[enrich]` to a new `[export]` section. Write tests for ExportConfig loading and defaults
...
## 15. Export `--all` Flag & Pipeline Command

- [ ] 15.01 — Add `--all` flag to the `export` command in `cli.py` (bool, default: False). When `--all` is passed, enable all available export formats (currently only `--docx`). `--all` satisfies the "at least one format flag" requirement — so `--all` alone is valid. `--all` combined with individual format flags is also valid (no conflict, `--all` simply enables everything). Update the format-flag validation logic: error only if none of `--all`, `--docx` (or future flags) are provided. Write tests: `--all` enables docx export, `--all` alone is valid (no "no format" error), `--all` combined with `--docx` works without conflict
- [ ] 15.02 — Implement `pipeline` command in `cli.py` with a single option: `--config` (TOML config path, default: ./config.toml). The command sequentially executes three stages: (1) `run` — load `TransskriboConfig`, set up logging, check ffprobe, run the full transcription pipeline (reuse `_run_pipeline` internals); (2) `enrich` — load `EnrichConfig`, batch-enrich all result JSONs in `output_dir` (reuse `_enrich_batch` internals, skip already-enriched); (3) `export --all` — load `ExportConfig`, batch-export all enriched result JSONs in all available formats (reuse `_export_batch` internals, skip already-exported). Each stage proceeds regardless of partial failures in the previous stage. Log a per-stage summary after each stage completes. Log a final combined summary at the end showing counts from all three stages
- [ ] 15.03 — Write tests for `pipeline` command: full pipeline calls all three stages in order, stage 2 (enrich) runs even if stage 1 (run) had partial failures, stage 3 (export) runs even if stage 2 had partial failures, config is loaded once and shared, per-stage summaries are logged, final combined summary is logged. Use mocks for `_run_pipeline`, `_enrich_batch`, and `_export_batch` internals (or their underlying functions) to avoid needing GPU/LLM in tests
- [ ] 15.04 — Update `README.md`: add `pipeline` command to Usage section as the recommended one-liner for full end-to-end processing (place it first, before the individual commands). Add `--all` flag example to the export section. Add `pipeline` to the Features list

**Done when:** `transskribo export --all` exports in all available formats. `transskribo pipeline --config config.toml` runs the full end-to-end workflow (transcribe → enrich → export all) in a single command. Partial failures in any stage do not prevent subsequent stages from running. README documents the new commands. Tests pass, lint clean, types clean.
